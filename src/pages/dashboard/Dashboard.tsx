...
export default function Dashboard() {
 ...
 const fetchInboundCards = async () => {
  const response: AxiosResponse<CardsAPIResponseType> = await client.get(
   `${ENDPOINTS.inboundCards}`
  )
  setCards(response.data.data)
 }

 const fetchStagesData = async () => {
  const response: AxiosResponse<StagesAPIResponseType> = await client.get(
   `${ENDPOINTS.stages}`
  )
 ...
 return (
  <div className=''>
   <div className='min-h-[110vh]'>
    <p className='text-xl pb-3'>Overview</p>
